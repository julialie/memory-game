...
// shuffleCards(); - other parts - might be easier to work with unshuffled cards
 ...
 */
 ...
 // max two cards, will be removed straight away. Cards appear but not matched
 let openCards = [];
 // this only remains useful for console logging.
 let matchedCards = [];
 //this informs when the game is won
 let matchedCount = 0;
 // number of moves the player makes - counts when cards are matched and when they are hidden
 let moveCount = 0;
...
function checkIfWon () {
    if (matchedCount === 8) {
        // alert("Congrats! You completed the game in "+moveCount+" moves!");
        stopTimer();
...
    // ensure modal is not displaying
    successModal.style.display = 'none';
    stopTimer();
    displayTimer.textContent = "0:00";
    // Reset move count
